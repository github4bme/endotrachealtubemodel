...
def move_servo1_right():
    inc = global_inc
    if kit.servo[0].angle + inc <= 180:
        kit.servo[0].angle += inc
    else:
        print("limit reached")

def move_servo1_left():
    inc = -global_inc
    if kit.servo[0].angle + inc >= 0:
        kit.servo[0].angle += inc
    else:
        print("limit reached")
...
def move_servo2_right():
    inc = global_inc
    if kit.servo[1].angle + inc <= 180:
        kit.servo[1].angle += inc
    else:
        print("limit reached")

def move_servo2_left():
    inc = -global_inc
    if kit.servo[1].angle + inc >= 0:
        kit.servo[1].angle += inc
    else:
        print("limit reached")
...
try:
    while True:
        ...
        print("Servo 1 angle: " + str(kit.servo[0].angle))
        print("Servo 2 angle: " + str(kit.servo[1].angle))
